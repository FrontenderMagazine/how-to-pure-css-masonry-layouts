...
Добавив колонки в свою разметку, вы получите больше контроля над разметкой. В любом случае, вам нужно хрошенько обдумать, как и где будут находится элементы вашего контента. Увидеть подобную реализацию в действии можно [здесь][15].

Использование свойства multi-column в разметке.
===

Для получения базовой "кирпичной кладки", на мой взгляд, свойство multi-column подходит больше всего. Мое мнение основывается на простоте и минимуме разметки. И в итоге, все работает. Если базовой "кладки" достаточно, то нижепреведенный код - это все, что вам нужно. У каждого из подходов есть свои преимущества.
...
Сделав небольшой рефакторинг, и наши панели стали блоками сами по себе. В итоге у нас будет вто такой код:
...
Использование вложеных кластеров контента помогает разбить контент на блоки и изменить стандартный поток, что нам и нужно. В итоге наш контент будет выглядеть разбитым на колонки.

Вложеные кластеры контента по сути это разметка в разметке, которай может "течь" и вертикально, и горизотально. И в данном случае,  Flexbox отличное решение.
...
И это неправильно. Указывание конкретных размеров, может не сработать в каждом конкретном случае. Лучшим вараинтом будет использование колоночных контейнеров, которые мы обсудили ранее. Давайте для примера возьмем такую раскладку:
...
Приятным моментом такой реализации будет факт, что количество рядов и колонок расположится в соответствии с вашим контетом.
